...
func (s *studentRepoImpl) FetchAll() ([]model.Student, error) {
	...
	row, err := s.db.Query("SELECT * FROM students")
	if err != nil {
		return nil, err
	}

	for row.Next() {
		var staudents model.Student

		err := row.Scan(&staudents.ID, &staudents.Name, &staudents.Address, &staudents.Class)
		if err != nil {
			return nil, err
		}

		listStudents = append(listStudents, staudents)
	}
